...
service.importTownData = async (modelsService, user, townIdOrName, fileName) => {
  ...
  const removeOldDraft = async () => {
    const oldDraft = await Draft.findOne({ town: town._id, autogenerated: true });
    if (oldDraft) {
      town.drafts = town.drafts.filter(d => !d._id.equals(oldDraft._id));
      await town.save();

      await Draft.deleteOne({ _id: oldDraft._id });
      await Line.deleteMany({ draft: oldDraft._id });
      await Station.deleteMany({ draft: oldDraft._id });
      await Connection.deleteMany({ draft: oldDraft._id });
    }
  }
