...
service.exportDraftData = async (modelsService, exportId) => {
  const draft = await modelsService
    .getModel("Draft")
    .findOne({ exportId: exportId })
    .populate({ path: "town", select: "url" });
  if (!draft) {
    return { statusCode: 404, data: "Draft does not exist" };
  }

  const stations = await modelsService
    .getModel("Station")
  ...
  try {
    ...
    // Stations
    const stationsSheet = XLSX.utils.json_to_sheet(
      stations.map(s => {
        return {
          name: s.name,
          year: s.year,
    ...
    // Lines
    const lines = await modelsService
      .getModel("Line")
      .find({ draft: draft._id })
      .sort("order")
...
service.importDraftData = async (modelsService, user, draftId, fileName) => {
  ...
  const generateStation = async (station, draftId) => {
    ...
    return await stationService.addStation(
      modelsService,
      user,
      draftId,
      stationObj
  ...
  const generateConnection = async (
    ...
    const stationFromName =
      connection["station_from"] || prevConnection["station_to"];
    const stationA = stationDocuments.find(s => s.name === stationFromName);
    const stationB = stationDocuments.find(
      s => s.name === connection["station_to"]
    );
  ...
  try {
    ...
    for (let sheetName of Object.keys(book.Sheets)) {
      ...
      if (sheetName.startsWith("Line_")) {
        const lineRes = await generateLine(
          XLSX.utils.sheet_to_json(book.Sheets[sheetName]),
          draft._id
        );
        // Line connections
        const connections = XLSX.utils
          .sheet_to_json(book.Sheets[sheetName])
          .slice(1);
        for (let i = 0; i < connections.length; i++) {
          await generateConnection(
            lineRes.data,
            connections[i],
            i > 0 ? connections[i - 1] : null,
            draft._id
          );
        }
      }
...
service.importTownsImages = async (modelsService, user, files) => {
  ...
  const uplodadAndSaveUrl = async (file, folder) => {
    ...
    file.mv(tempFilePath, async err => {
      ...
      town[folder] = await storage.uploadAndGetUrl(
        tempFilePath,
        `/${folder}/${file.name}`
      );
      await town.save();
...
service.doCalculations = async (modelsService, user, draftId) => {
  ...
  const calculateStationsAndDistanceInLine = async Line => {
    const lines = await Line.find({}).populate({
      path: "connections",
      populate: { path: "stations", select: "id" }
    });
