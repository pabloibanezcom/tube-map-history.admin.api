...
// ASSIGN TOWN ROLE TO USER
// PUT /api/user/town/role/:userId/:town
describe('ASSIGN TOWN ROLE TO USER', () => {
  let tokenM1;
  let tokenA;
  let userA;
  ...
  it('when user is not logged it can not assign town role', async (done) => {
    agent.put(`/api/user/town/role/${userA._id}/london`).send({ role: 'manager' }).set('Accept', 'application/json')
      .expect(401, done);
  });

  it('when user is manager it can not assign town role', async (done) => {
    agent.put(`/api/user/town/role/${userA._id}/london`).send({ role: 'manager' }).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenM1}`)
      .expect(401, done);
  });

  it('when user is admin it can assign town role to any user', async (done) => {
    await agent.put(`/api/user/town/role/${userA._id}/london`).send({ role: 'manager' }).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenA}`)
      .expect(200);
    const res = await agent.get(`/api/user/${userA._id}`).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenA}`)
      .expect(200);
    expect(res.body.towns.find(t => t.town.name === 'London').role).toBe('manager');
    done();
  });
});