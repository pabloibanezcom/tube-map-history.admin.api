...
describe('SEARCH LINES', () => {
  ...
  it('when filter name is defined returns only lines with name containing it', async (done) => {
    const response = await agent.post(`/api/${draft._id}/line/search`).send({ ...lineSearchBody, filter: { ...lineSearchBody.filter, name: 'piccadilly ' } }).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenA}`);
...
describe('ADD LINE', () => {
  ...
  it('when user is not town manager it can not add line', async (done) => {
    agent.post(`/api/${draft._id}/line`).send(mockLine).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenU}`)
      .expect(401, done);
  });

  it('when user is town manager it can add line', async (done) => {
    agent.post(`/api/${draft._id}/line`).send(mockLine).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenM1}`)
      .expect(200, done);
  });
...
describe('UPDATE LINE', () => {
  ...
  beforeAll(async (done) => {
    tokenM1 = await loginAsRole('M1');
    const lineRes = await agent.post(`/api/${draft._id}/line`).send(mockLine).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenM1}`).expect(200);
    line = lineRes.body;
    done();
  });
...
describe('DELETE LINE', () => {
  ...
  beforeAll(async (done) => {
    tokenM1 = await loginAsRole('M1');
    const lineRes1 = await agent.post(`/api/${draft._id}/line`).send(mockLine).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenM1}`).expect(200);
    line1 = lineRes1.body;
    const lineRes2 = await agent.post(`/api/${draft._id}/line`).send(mockLine).set('Accept', 'application/json').set('Authorization', `Bearer ${tokenM1}`).expect(200);
    line2 = lineRes2.body;
    done();
  });
